        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            @foreach ($fakultas as $item)
                <a href="{{ route('rps.prodi', $item->slug) }}"
                    class="group relative bg-card/80 border border-border/50 rounded-3xl p-6 overflow-hidden hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 text-center block">
                    ...
                    <div class="relative z-10">
                        <!-- Logo Fakultas (ukuran lebih kecil agar card tidak terlalu besar) -->
                        <div class="mb-5 mx-auto w-28 h-28 flex items-center justify-center">
                        ...
                        <!-- Nama Fakultas (Tengah) -->
                        <h3
                            class="text-xl font-heading font-bold text-foreground mb-4 group-hover:text-primary transition-colors duration-300">
                            {{ $item->name }}
                        </h3>

                        <!-- Deskripsi: Full tanpa limit, rata kiri -->
                        <div class="text-sm text-muted-foreground mb-6 leading-relaxed text-left">
                            {!! $item->deskripsi !!}
                        </div>
                        ...
                        <p class="text-sm text-muted-foreground mb-6">
                            <span class="font-bold text-base text-primary">
                                {{ $item->prodi_count ?? $item->prodis->count() }}
                            </span>
                            Program Studi
                        </p>
                    </div>
            ...
            @if ($fakultas->isEmpty())
                <div class="col-span-full text-center py-20">
                    ...
                    <p class="text-lg text-muted-foreground max-w-md mx-auto">
                        Data fakultas akan segera ditambahkan.
                    </p>
                </div>
            @endif
        </div>
