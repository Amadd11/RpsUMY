@php
    $totalBobot = $rps->rencanas->sum('bobot');
@endphp
...
<div class="bg-card rounded-3xl p-6 md:p-8 shadow-xl border border-border/50 space-y-8">
    ...
    <!-- Scrollable Table -->
    <div class="overflow-hidden border border-border/50 rounded-[2rem] shadow-sm bg-white/50 ">
            ...
            <div class="min-w-[1400px]">
                <table class="w-full text-sm text-left border-collapse">
                    <thead>
                        <!-- Main Header Row -->
                        <tr
                            class="bg-gradient-to-r from-indigo-50 to-purple-50/50 text-indigo-900 border-b-2 border-indigo-200/50">
                            <th rowspan="2"
                        ...
                        <!-- Subheader Row -->
                        <tr
                            class="text-indigo-900 bg-gradient-to-r from-indigo-100/50 to-amber-100/50 border border-indigo-200/50">
                            <th
                            ...
                            <th
                                class="w-40 p-3 font-bold  tracking-wider text-sm text-center border border-indigo-200/50">
                                Dari <span class="text-purple-600 font-black"></span>
                            </th>
                        </tr>
                    </thead>

                    <tbody class="divide-y divide-border/40">
                        @forelse ($rps->rencanas->sortBy('week') as $index => $rencana)
                            <tr
                                ...
                                <td class="p-4 text-center align-top border border-indigo-200/30 font-black">
                                    <div
                                        ...
                                        <span
                                            class="text-[9px]  tracking-tighter leading-none mb-0.5 text-indigo-600"></span>
                                ...
                                <!-- Indikator -->
                                <td class="p-4 align-top border border-indigo-200/30">
                                    @include('components.expandable-content', [
                                        'content' => $rencana->indikator ?? '-',
                                        'buttonColor' => 'blue',
                                        'threshold' => 120,
                                    ])
                                </td>
                                ...
                                <td class="p-4 align-top border border-indigo-200/30">
                                    @include('components.expandable-content', [
                                        'content' => $rencana->kriteria_teknik ?? '-',
                                        'buttonColor' => 'amber',
                                        'threshold' => 120,
                                    ])
                                </td>
                                ...
                                <td class="p-4 align-top border border-indigo-200/30 text-sm">
                                    @include('components.expandable-content', [
                                        'content' =>
                                            $rencana->dari ??
                                            ($rencana->daring ?? ($rencana->materi_pembelajaran ?? '-')),
                                        'buttonColor' => 'purple',
                                        'threshold' => 120,
                                    ])
                                </td>
                                ...
                                <td class="p-4 align-top border border-indigo-200/30">
                                    @include('components.expandable-content', [
                                        'content' => $rencana->materi_pembelajaran ?? '-',
                                        'buttonColor' => 'slate',
                                        'threshold' => 120,
                                    ])
                                </td>
                        ...
                        <!-- Total Bobot Row -->
                        @if ($rps->rencanas->isNotEmpty())
                            <tr
                                class="bg-gradient-to-r from-indigo-100/50 via-purple-100/50 to-emerald-100/50 font-bold text-indigo-800 border-t-2 border-indigo-200/50">
                                <td colspan="8"
                                    class="p-4 tracking-wide text-center  text-lg font-black border-t border-indigo-200/50">
                                    Total Bobot Penilaian</td>
                                <td
                                    class="p-4 text-xl font-black text-center text-emerald-700 border-t border-indigo-200/50">
                                    {{ $totalBobot }}%</td>
                            </tr>
                        @endif
                    </tbody>
    ...
    <footer class="flex items-center justify-between p-5 bg-indigo-50/50 rounded-2xl border border-indigo-100/50">
        <div class="flex items-center gap-3">
            <iconify-icon icon="solar:info-square-bold" class="size-5 text-indigo-600 flex-shrink-0"></iconify-icon>
